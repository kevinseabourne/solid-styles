...
async function findProjectRoot() {
  ...
  if (isPostinstall) {
    ...
    // Traverse upward until we exit node_modules
    while (currentDir.includes('node_modules') && currentDir !== dirname(currentDir)) {
      currentDir = dirname(currentDir);
    }
    
    // Now we should be at the project root
    let projectRoot = currentDir;
    
    // Special handling for pnpm structure: node_modules/.pnpm/package@version/node_modules/package
    if (projectRoot.includes('.pnpm')) {
      // For pnpm, we need to go up further to find the real project root
      while (projectRoot.includes('node_modules') && projectRoot !== dirname(projectRoot)) {
...
async function detectFramework(rootDir) {
  try {
    ...
    // SolidStart detection (higher priority)
    if (deps['@solidjs/start'] || deps['vinxi']) {
      log('🎯 Auto-detected: SolidStart project', 'cyan');
      return 'solidstart';
    }
    
    // Vite + Solid.js detection  
    if (deps['vite'] && deps['solid-js']) {
      log('🎯 Auto-detected: Solid.js with Vite project', 'cyan');
      return 'solid';
    }
    
    // Check for config files as fallback
    if (await fileExists(resolve(rootDir, 'app.config.ts')) || 
        await fileExists(resolve(rootDir, 'app.config.js'))) {
      log('🎯 Auto-detected: SolidStart project (via config file)', 'cyan');
      return 'solidstart';
    }
    
    if (await fileExists(resolve(rootDir, 'vite.config.ts')) || 
        await fileExists(resolve(rootDir, 'vite.config.js'))) {
      log('🎯 Auto-detected: Solid.js project (via vite config)', 'cyan');
      return 'solid';
    }
    
    // Default to SolidStart (most common)
    log('🎯 No specific framework detected, defaulting to SolidStart', 'yellow');
    return 'solidstart';
  } catch (error) {
    log('⚠️  Framework detection failed, defaulting to SolidStart', 'yellow');
    return 'solidstart';
  }
}

async function promptUser(rootDir) {
  ...
  try {
    ...
    if (await fileExists(configPath)) {
      ...
      let reconfigure;
      try {
        reconfigure = await new Promise((resolve) => {
          rl.question(
...
async function configureForSolid(rootDir) {
...
async function configureForSolidStart(rootDir) {
...
async function detectPackageManager(rootDir) {
  ...
  if (await fileExists(resolve(rootDir, 'pnpm-lock.yaml'))) {
    ...
    packageManager = 'yarn';
  } else if (await fileExists(resolve(rootDir, 'bun.lockb'))) {
    packageManager = 'bun';
  }
...
async function main() {
  try {
    // Add debugging info
    console.log('🔧 solid-styles setup starting...');
    console.log(`📍 Process: ${process.argv0}`);
    console.log(`📂 CWD: ${process.cwd()}`);
    console.log(`🖥️  TTY: ${process.stdout.isTTY}`);
    ...
    // Auto-detect framework or use interactive prompt
    let framework;
    if (!process.stdout.isTTY) {
      // Non-interactive environment (postinstall) - use auto-detection
      log('🔍 Auto-detecting framework...', 'cyan');
      framework = await detectFramework(rootDir);
    } else {
      // Interactive environment - allow user choice
      framework = await promptUser(rootDir);
    }
    ...
    if (framework === 'solid') {
      await configureForSolid(rootDir);
    } else {
      await configureForSolidStart(rootDir);
    }
    ...
    const frameworkName = framework === 'solid' ? 'Solid.js with Vite' : 'SolidStart';
    
    log('\n✨\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728', 'green');
    log(`🎉 solid-styles successfully configured for ${frameworkName}!`, 'bright');
    log('\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\u2728\n', 'green');
    
    log('🚀 Your project is now ready with:', 'cyan');
    log('  ✓ Modern theme system with CSS variables', 'green');
    log('  ✓ Zero-runtime styled components', 'green');
    log('  ✓ Lightning CSS optimization', 'green');
    log('  ✓ Spring animations ready', 'green');
    
    log('\n📚 Quick start commands:', 'cyan');
    log(`  ${packageManager} run dev     - Start development server`, 'green');
    log(`  ${packageManager} run build   - Build your project`, 'green');
    
    log('\n📄 Files created:', 'cyan');
    log('  🎨 src/styles/theme.ts - Theme configuration', 'yellow');
    log('  🎨 src/styles/global.css - Global styles', 'yellow');
    log('  🧩 src/components/ThemeProvider.tsx - Theme context', 'yellow');
    
    log(`\n⚙️  To reconfigure: npx solid-styles setup`, 'yellow');
  } catch (error) {
